...
function init(){	
	...
	//var orderId = url.substring(startPos+1,endPos)*1;
	
	var srcStr = window.location.href;
	var orderId = getParamFromUrl(srcStr,"orderId");
	
	getResultInfo(orderId);
} 
...
function getResultInfo(orderId){

	 var func = function(objs){
	 	workFlowCheck.fillTalbe(objs,workFlowCheck);
	 }
	workFlowCheck.getOaWorkFlowCheckByOrderId(orderId,func);
}
