...
Ext.define("Org.fileupload.Panel", {
	...
	initComponent : function() {
		...
		this.setting={
				...
				file_types : this.fileTypes||"*.*",  //允许上传的文件类�? 
		        file_types_description : "All Files",  //文件类型描述
		        file_upload_limit : "0",  //限定用户�?次�?�最多上传多少个文件，在上传过程中，该数字会累加，如果设置为�?0”，则表示没有限�? 
		        file_queue_limit : "10",//上传队列数量限制，该项�?�常不需设置，会根据file_upload_limit自动赋�??              
				post_params : this.postParams||{savePath:'upload\\'},
				use_query_string : true,
				debug : true,
				button_cursor : SWFUpload.CURSOR.HAND,
				button_window_mode : SWFUpload.WINDOW_MODE.TRANSPARENT,
				custom_settings : {//自定义参�?
					scope_handler : this
				},
				swfupload_loaded_handler : function(){
					console.log("swf组件成功初始�?");
				},// 当Flash控件成功加载后触发的事件处理函数
				file_dialog_start_handler : function(){},// 当文件�?�取对话框弹出前出发的事件处理函�?
				file_dialog_complete_handler : function(){},//当文件�?�取对话框关闭后触发的事件处�?
				upload_start_handler : function(){},// �?始上传文件前触发的事件处理函�?
					...
					var rightClick = Ext.create('Ext.menu.Menu', {
										...
										items : [{
													...
													handler:function(){
														...
														label.setText(label.text="队列中文件个�?:"+stats.files_queued);
													},
		...
		this.tbar=[
			{text:'添加文件',id:"btnAdd",icon:"image/icons/add.png"},'-',
			{text:'上传全部未开始上传文�?',handler:function(){
		...
		this.listeners={
			'afterrender':function(){
				console.log("渲染完成�? 添加swf�?�?的设�?");
				var em=Ext.get(Ext.query("#btnAdd>em")[0]);
				if(!em){
					//此处为IE9�?下版本的兼容问题的临时解决办法，目前还是不支持IE6
	...
	formatFileState : function(n){//文件状�??
		switch(n){
			case SWFUpload.FILE_STATUS.QUEUED  : return '已加入队�?';
			break;
	...
	onFileError : function(file,errorCode,msg){
		...
		switch(errorCode){
			...
			break;
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT : msg='文件太大，不能�?�择！文件大小不能超�?'+this.settings.file_size_limit/1024+'MB';
			break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE : msg='该文件大小为0，不能�?�择�?';
			break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE : msg='该文件类型不可以上传�?';
			break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED : msg="上传已经停止";
			break;
			case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:msg="�?有文件已经取消！";
			break;
			default:msg="未知错误!";
			break;
		}
	...
	uploadProgress : function(file, bytesComplete, totalBytes){//处理进度�?
		console.log("完成百分�?"+file.percentUploaded+"，当前�?�度"+file.currentSpeed/8/1024/1024+"MB/s");
