...
function init(){
	...
	$("caption").value = "载体年度统计";
	
	//如果 args2="" 则不显示
	var  args2 ="投放金额,到帐金额,指标,完成比例,到账比例";
...
//	var srcStr = decodeURI(window.location.search);
	...
	var isNotReturnValue = getParamFromUrl(srcStr,"isNotReturnValue");
	var userName = getParamFromUrl(srcStr,"userName");
	var channelModelParam = getParamFromUrl(srcStr,"channelModelParam");
	var purpose = getParamFromUrl(srcStr,"purpose");
	
	FusionChartsManager.getCarrierTargetChartObjs(userId,channelModelParam,year,startDate,endDate,isPutYear,isNotReturnValue,carrierId,userName,purpose,null,func);
