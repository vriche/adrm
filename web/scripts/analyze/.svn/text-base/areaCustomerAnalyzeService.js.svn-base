...
//	var userId = $(user.selectName).value==0?null:$(user.selectName).value;
	...
	if(beginDate != '' && endDate != ''){
		...
		if(carrierIds==null){
			...
			Ext.getBody().mask('数据加载中……', 'x-mask-loading');
			analyzeClass.getAreaCustomerByCarrierTypeXML(carrierIds,0,beginDate,endDate,userId,userName,isPrint,func);
		}
