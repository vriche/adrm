...
function init(){
	...
	orderCategory.obj.orgId = $("orgId").value;
 	orderCategory.obj.version = $("resource_year").value;
  	orderCategory.makeSubCategoryFromOrder("orderCategoryIdDiv","orderCategoryId",120,function(){});	
...
function initGrid(){
	...
	var flds = "载体名称,时段名称,客户名称,投放金额,分配金额,投放时长";
	
	mygrid.setHeader(flds);


	mygrid.setInitWidthsP("15,29,26,10,10,10");


	mygrid.setColAlign("left,left,left,right,right,right");
	mygrid.setColTypes("ed,tree,ed,ed,ed,ed");
...
//	$("carrierNameForm").value = $("carrierName").value==0?null:$("carrierName").value;
	...
	$("weekStrForm").value=  Ext.getCmp('orderCategoryId').getValue();
	...
	reportForm.action="reports/jsp/resourceAnalyze_reports.jsp";
	reportForm.submit(); 	
}
...
function search(){
	...
	var orderSubCategorys =  Ext.getCmp('orderCategoryId').getValue();
...
//			orderdayinfo.getResourceByDate(orderdayinfo,userId,userName,isPrint);  
			orderdayinfo.getResourceListByDateXML(startDate,endDate,resourceIds,userId,userName,isPrint,orderSubCategorys,weekStr,func);
	    }else{
	        alert("请选择时段后进行查询");
	    }
	}else{
...
//startDate,endDate,resourceIds,userId,userName,isPrint
	if($("beginDate").value=="" || $("overDate").value=="" ){
		...
		var a = {
                ...
                userName: userName,
                orderSubCategorys: orderSubCategorys,
                isPrint: isPrint
		};		
