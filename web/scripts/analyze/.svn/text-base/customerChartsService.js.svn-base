...
//	var srcStr = decodeURI(window.location.search);
	...
	var sorCol = getParamFromUrl(srcStr,"sorCol");
	var sorType = getParamFromUrl(srcStr,"sorType");
	...
	var weekStr = getParamFromUrl(srcStr,"weekStr");
	if(resourceTypeId =='')	    resourceTypeId = "0";        

	
	FusionChartsManager.getCustomerChartObjs(orgId,resourceTypeId,startDate,endDate,sorCol,sorType,putYear,userId,carrierName,customerId,channelModelParam,theUser,incomPurs,returnValue,weekStr,func);
