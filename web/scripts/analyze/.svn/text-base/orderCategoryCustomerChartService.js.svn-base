...
function init(){
	...
	$("caption").value = "订单类别客户统计";
	
	//如果 args2="" 则不显示
	var  args2 ="刊例总价,投放金额,优惠金额,投放时长";
...
//	var srcStr = decodeURI(window.location.search);
	...
	FusionChartsManager.getOrderCustomerChartObjs(carrierIds,channelModelParam,beginDate,endDate,userId,userName,isPrint,func);
...
function renderFromQS(objs,name,divName){
	...
	FCharts.LabelsData.cols =["订单类别","刊例总价","投放金额","优惠金额","投放时长"];
	//显示列
