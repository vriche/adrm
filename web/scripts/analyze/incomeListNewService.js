...
function init(){
    ...
    _make_org_select("orgId",100,"loadGridData");	
    orgId = $("orgId").value;
//    $("fenpeiInfo").value = 2;
...
function initGrid5(){
	...
	var flds = "频道*时段属性*时段*段位描述*分配*比率";
	mygrid5.setHeader(flds);
	
	mygrid5.setColspan(0); 
	
	var columnIds = "filed0*filed1*filed2*filed3*filed4*filed5";
	mygrid5.setColumnIds(columnIds);

	mygrid5.setInitWidthsP("15*15*5*25*20*20");         
...
//  var carrierCmd = carrier.getLovCombo('carrier_cmd',true);      
	var channelCmd = channel.getLovCombo('channel_cmd',180,'remote',true); 
...
//	      var channelCmd = channel.getLovCombo('channel_id',180,'local',false);    
        carrier.obj.orgId = config_oneOrgMoreSuborgsParam == '1'?1:$("orgId").value;	
	      var carrierCmd = carrier.getLovCombo2('carrier_id',180,'local',false);     
...
//			customer.customerCommand.setValue(o.customerId);
	...
	//			  Ext.getCmp('channel_id').setValue(o.resourceChannelId);
			...
			if(o.incomeUsed == 0 || o.incomeUsed == null){
	 				  incomeIdCmp.enable(); 
