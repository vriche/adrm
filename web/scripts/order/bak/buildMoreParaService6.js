...
function build_grid_menuStr(){
	...
	sb = sb + "<item text=\"漂移\" img=\"red.gif\"  id=\"copy_move\"/>";
	sb = sb + "<item text=\"复制\" img=\"red.gif\"  id=\"copy\"/>";
	sb = sb + "<item text=\"粘贴\" img=\"red.gif\"  id=\"post\">";
	...
	sb = sb + "<item text=\"复制并新添\" img=\"red.gif\"  id=\"copy_post\"/>";	

	sb = sb + "<item text=\"删除\" img=\"red.gif\"  id=\"remove\"/>";
 ...
 function initGrid1(){
 	...
 	function onButtonClick(menuitemId, type) {
	    ...
	    if(menuitemId =='copy'|| menuitemId =='copy_post'|| menuitemId =='copy_move'){
...
// 		 	showAutoPrice_prompt_whith_comboBox(funcc);
	    ...
	    if( menuitemId =='post_pos' || menuitemId =='post_edit' || menuitemId =='post_arr' || menuitemId =='post_all' || menuitemId =='copy_post'|| menuitemId =='copy_move'){
	    	...
	    	for(var i = 0;i<mygrid1.selectedRows.length;i++){
	    		...
	    		var row_id = menuitemId =='copy_post'||'copy_move'?addGrid1NewRow(menuitemId,copy_grid1_rowid):mygrid1.selectedRows[i].idd;
...
//			columnIds = "resourceName,edit,len,apppos,start,end,stantPrice,realPrice,favace,appRae,sumTime,sumPay"
			...
			if(this.getColumnId(cellIndex)=='resourceName'){
				displayCompagesTree2(1,wasChanged);return false; 
				 ...
				 search_adver_cont(1,wasChanged);return false;  
...
//		return false
 	...
 	function onLetfClick(parentNodeId,_cellIndex,ev){
 		mygrid2onKeypressClick(ev);
// 		mygrid2OnKeyPressed(ev.keyCode,ev.ctrlKey,ev.shiftKey,ev)
 	}
    mygrid2.attachEvent("onKeyPress", mygrid2OnKeyPressed);
    mygrid2.attachEvent("onRightClick", onRightClick);
//    mygrid2.attachEvent("onLeftClick", onLetfClick);
...
function mygrid2onKeypressClick_bak(ev){
//			alert(111)
		if(ev.button == 0){ev.keyCode = 49;}
		if(ev.button == 2){ev.keyCode = 48;}
		var keyCode = ev.keyCode;
