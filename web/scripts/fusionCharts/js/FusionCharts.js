	...
	render: function(elementId){
		...
		if((this.detectFlashVersion==1) && (this.installedVer.major < 8)){
			if (this.autoInstallRedirect==1){
				...
				var installationConfirm = window.confirm("此功能需要先安装fresh play v8.0或更高版本,请确认是否现在下载,下载安装后才可正常使用!");
				if (installationConfirm){
					//window.location = "http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash";
					 //window.location = "/swflash.cab#version=8,0,0,0";
//					window.location =contPath +"download/install_flash_player.exe";
					window.location = contPath +"download/install_freshPlay.jsp";
				}else{
					return false;
				}
			}else{
