...
//	var srcStr = decodeURI(window.location.search);
	...
	var yearForm = getParamFromUrl(srcStr,"yearForm");
...
//	var userName = getParamFromUrl(srcStr,"userName");
	...
	FusionChartsManager.getFinanceTargetChartObjs(yearForm,userName,carrierId,channelModelParam,func);
...
function renderFromQS(){
	...
	//Data 及 lable 
	//列名
	FCharts.LabelsData.cols =["频道名称","一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"];
	//显示列
