...
function dhtmlXGridObject(id) {
	...
	this.onCheckbox = this.onEnter = this.onRowSelect = this.onEditCell = function() {
		return true;
	};
	...
	this._key_events = {
		...
		k40_0_0 : function() {
			...
			if (this.editor && this.editor.combo)
				...
				if (!this.row.idd)
					return;
				var rowInd = Math.max((master._r_select || 0), this
						.getRowIndex(this.row.idd)) + 1;
	...
	this._nextRowCell = function(row, dir, pos) {
		// if(this.rowsBuffer == '') this.rowsBuffer =new Array(new
		// dhtmlxArray(0),new dhtmlxArray(0));
	...
	this.checkAll = function() {
		for (var cInd = 0; cInd < this.getColumnsNum(); cInd++) {
			if (this.getColType(cInd) == "ch")
				this.setAllChecks(cInd, 1)
		}
	};
	this.uncheckAll = function() {
		for (var cInd = 0; cInd < this.getColumnsNum(); cInd++) {
			if (this.getColType(cInd) == "ch")
				this.setAllChecks(cInd, "")
		}
	};
	this.setAllChecks = function(cInd, v) {
	...
	this.doOnScroll = function(e, mode) {
		...
		if (!this.pagingOn
				&& this.objBox.scrollTop + this.hdrSizeA
						+ this.objBox.offsetHeight > this.objBox.scrollHeight) {
			if (this._xml_ready
	...
	this.setSizes = function(fl) {
		if ((!this.noHeader)
				&& ((!this.hdr.rows[0]) || (!this.hdrBox.offsetWidth)))
			return;
		...
		if (this._ahgr)
			if (this.objBox.scrollHeight) {
				...
				var scrfix = ((this.objBox.offsetWidth < this.objBox.scrollWidth) ? (_isFF ? 20
						: 18)
						: 1);
				if (this._ahgrMA)
		...
		if (!this.noHeader)
			aRow.style.top = (zheight - this.hdrBox.offsetHeight + (_isFF ? 0
					: (1 - wcor)))
					+ "px";
		...
		if (this._dload)
			this._dloadSize = Math
					.floor(parseInt(this.entBox.style.height) / 20) + 2;
	...
	this._doContClick = function(ev) {
		...
		if (ev.button == 0) {
			if ((this.onLCL)
					&& (!this.onLCL(el.parentNode.idd, el._cellIndex, ev)))
				return;
		}
		if (ev.button == 2) {
			if ((this.onRCL)
					&& (!this.onRCL(el.parentNode.idd, el._cellIndex, ev)))
				return;
			if (this._ctmndx) {
				if ((this.onBCM)
						&& (!this.onBCM(el.parentNode.idd, el._cellIndex, this)))
					return true;
	...
	this.doClick = function(el, fl, selMethod) {
		...
		if (this.cell != null)
			this.cell.className = this.cell.className.replace(/cellselected/g,
					"");
		if (el.tagName == "TD"
				&& (this.rowsCol._dhx_find(this.rowsAr[el.parentNode.idd]) != -1 || this.rowsBuffer[0]
						._dhx_find(el.parentNode.idd) != -1)) {
			if (this.onSSC)
			...
			if (selMethod == 0) {
				...
				for (var i = 0; i < this.rowsCol.length; i++) {
					if ((i >= strt && i <= end) && (this.rowsCol[i])
							&& (!this.rowsCol[i]._sRow)) {
						if ((!this.onBFS)
								|| (this.onBFS(this.rowsCol[i].idd, psid))) {
	...
	this.editStop = function() {
		...
		if (this.editor && this.editor != null) {
			...
			var g = this.editor;
			this.editor = null;
			var z = this.onEditCell(2, this.row.idd, this.cell._cellIndex, g
					.getValue(), g.val);
	...
	this.doKey = function(ev) {
		...
		if (this._htkebl)
			return true;
		if ((this.onKPR) && (!this.onKPR(ev.keyCode, ev.ctrlKey, ev.shiftKey)))
			return false;
		try {
			...
			if (ev.keyCode == 13 && (ev.ctrlKey || ev.shiftKey)) {
				...
				if (window.event.ctrlKey && rowInd != this.rowsCol.length - 1) {
					if (this.row.rowIndex == this.obj._rowslength() - 1
			...
			if (ev.keyCode == 9 && !ev.shiftKey) {
				...
				if (arow._childIndexes)
					while (arow._childIndexes[aind + 1] == arow._childIndexes[aind])
						aind++;
				...
				if (arow._childIndexes)
					while ((aind >= 0)
							&& (arow._childIndexes[aind] == arow._childIndexes[aind + 1]))
						aind--;
				...
				if (aind < 0) {
					...
					if (arow._childIndexes)
						while (((arow._childIndexes[aind] != "0") && (!arow._childIndexes[aind]))
								&& (aind >= 0))
							aind--;
			...
			if (ev.keyCode == 40 || ev.keyCode == 38) {
				...
				if (this.editor && this.editor.combo) {
					...
					if (ev.keyCode == 38 && rowInd != 1) {
						...
						this.selectCell(this.obj._rows(rowInd - 2),
								this.cell._cellIndex, true);
					} else if (this.pagingOn && ev.keyCode == 38 && rowInd == 1
							&& this.currentPage != 1) {
						this.changePage(this.currentPage - 1)
						this.selectCell(
								this.obj.rows[this.obj.rows.length - 1],
								this.cell._cellIndex, true);
					} else if (ev.keyCode == 40
							&& rowInd != this.rowsCol.length
							&& rowInd != this.obj.rows.length - 1) {

						this.selectCell(this.obj._rows(rowInd),
								this.cell._cellIndex, true);
					} else if (this.pagingOn
							&& ev.keyCode == 40
							&& (this.row != this.rowsCol[this.rowsCol.length - 1]
									|| this.rowsBuffer[0].length > 0 || !this.recordsNoMore)) {
						this.changePage(this.currentPage + 1)
						this.selectCell(this.obj._rows(0),
								this.cell._cellIndex, true);
					}
				}
				_isIE ? ev.returnValue = false : ev.preventDefault();
			...
			if (ev.keyCode == 33 || ev.keyCode == 34) {
				...
				this.selectCell(this.getRowIndex(this.row.idd)
						+ this.rowsBufferOutSize * (ev.keyCode != 33 ? 1 : -1),
						this.cell._cellIndex, true);
	...
	this.sortRows = function(col, type, order) {
		...
		for (var i = 0; i < this.rowsCol.length; i++)
			arrTS[this.rowsCol[i].idd] = this.cells3(this.rowsCol[i], col)
					.getValue();
	...
	this._sortRows = function(col, type, order, arrTS) {
		...
		if (type == 'cus') {
			this.rowsCol[sort](function(a, b) {
				return self._customSorts[col](arrTS[a.idd], arrTS[b.idd],
						order, a.idd, b.idd);
			});
		} else
		...
		if (type == 'str') {
			...
			this.rowsCol[sort](function(a, b) {
				var aVal = Date.parse(new Date(arrTS[a.idd])
						|| new Date("01/01/1900"))
				var bVal = Date.parse(new Date(arrTS[b.idd])
						|| new Date("01/01/1900"))
				if (order == "asc")
					return aVal - bVal
				else
	...
	this.setXMLAutoLoading = function(filePath, bufferSize) {
		...
		this.rowsBufferOutSize = bufferSize || this.rowsBufferOutSize == 0 ? 40
	...
	this.addRowsFromBuffer = function(stopBeforeServerCall) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			if (this.rowsBuffer[1][0].tagName == "TR") {
				...
				this._insertRowAt(this.createRowFromXMLTag(rowNode), -1,
						this.pagingOn);
			}
	...
	this.createRowFromXMLTag = function(rowNode) {
		...
		var r = this
				._fillRowFromXML(this._prepareRow(rId), rowNode, tree, null);
	...
	this.wasDblClicked = function(ev) {
		...
		if (el) {
			...
			return ((this.onRowDblClicked) ? this.onRowDblClicked(rowId,
					el._cellIndex) : true);
		}
	}
	...
	this._onHeaderClick = function(e) {
		...
		if ((this.grid.onHeaderClick)
				&& (!this.grid.onHeaderClick(el._cellIndexS)))
			return false;
	...
	this.getRowId = function(ind) {
		...
		if (z)
			return z.idd;
		return (this.rowsBuffer[0][this._dload ? ind : (ind
				- this.rowsCol.length - 1)] || null);
	}
	...
	this.setCellId = function(row_id, ind, id) {
		this.cell = this.cellAll(row_id, ind);

		this.cell.id = id;
	}
	this.getCellId = function(row_id, ind) {
		this.cell = this.cellAll(row_id, ind);
	...
	this.cellAll = function(row_id, col) {
		if (arguments.length == 0)
	...
	this.cells4 = function(cell) {
		if (!cell._cellType)
			return eval("new eXcell_" + this.cellType[cell._cellIndex]
					+ "(cell)");
		else
			return eval("new eXcell_" + cell._cellType + "(cell)");
	}

	this.cells5 = function(cell, type) {
	...
	this.sortField = function(ind, repeatFl) {
		...
		if (el.tagName == "TH" && (this.fldSort.length - 1) >= el._cellIndex
			...
			if ((this.onCLMS) && (!this.onCLMS(ind, this, sortType)))
				return;
	...
	this.setStyle = function(ss_header, ss_grid, ss_selCell, ss_selRow) {
		...
		for (var i = 0; i < prefs.length; i++)
			if (_isIE)
				...
				this.styleSheet[0].insertRule(prefs[i] + "{"
						+ this.ssModifier[i] + "}", 0);
	}
	...
	this._manualXMLSerialize = function(r) {
		...
		for (var jj = 0; jj < r.childNodes.length; jj++) {
			...
			if ((!this._srClmn) || (this._srClmn[i]))
				out += "<cell>" + (z.firstChild ? z.firstChild.data : "")
						+ "</cell>";
			i++;
		}
		out += "</row>";
	...
	this._serializeRow = function(r) {
		...
		var style = r._attrs?r._attrs.style:"";
		
		
		if ((!r) || (r._sRow) || (r._rLoad)) {
			if (this.xmlSerializer)
				out += this.xmlSerializer
						.serializeToString(this.rowsBuffer[1][i]);
			else
...
//						+ this.UserData[r.idd].get(keysAr[ii]) + "</userdata>";
		...
		for (var jj = 0; jj < r.childNodes.length; jj++) {
			if ((!this._srClmn) || (this._srClmn[jj])) {
				...
				if (this._sAll)
					out += (cvx._aimage ? (" image='" + cvx._aimage + "'") : "")
							+ ">" + ((zxVal === null) ? "" : zxVal) + "</cell>";
				else
	...
	this._serialiseConfig = function() {
		var out = "<head>";
		for (var i = 0; i < this.hdr.rows[0].cells.length; i++) {
			out += "<column width='" + this.cellWidthPX[i] + "' align='"
					+ this.cellAlign[i] + "' type='" + this.cellType[i]
					+ "' sort='" + this.fldSort[i] + "' color='"
					+ this.columnColor[i] + "'>";
			
			if (this._asCDATA)
				out += "<![CDATA[" + this.hdr.rows[1].cells[i].childNodes[0].innerHTML + "]]>";
			else
			...
			for (var i = 0; i < this.rowsBuffer[1].length; i++) {
				if (this.rowsBuffer[1][i].tagName == "TR") {
					...
					if (!this._onlChAttr) {
						if (this._srClmn)
							out += this
									._manualXMLSerialize(this.rowsBuffer[1][i]);
						else if (!this.xmlSerializer)
							out += this.rowsBuffer[1][i].xml;
						else {
							out += this.xmlSerializer
									.serializeToString(this.rowsBuffer[1][i]);
	...
	this.dhx_eventCatcher = function(obj) {
		...
		var z = function() {
			...
			for (var i = 0; i < dhx_catch.length; i++)
				if (!dhx_catch[i].apply(m_obj, arguments))
					res = false;
			return res;
		}
	...
	this.obj.onclick = new Function(
			"e",
			"this.grid._doClick(e||window.event);if(this.grid._sclE)this.grid.editCell(e||window.event);(e||event).cancelBubble=true;");
	this.entBox.onmousedown = new Function("e",
			"return this.grid._doContClick(e||window.event);");
	this.obj.ondblclick = new Function(
			"e",
			"if(!this.grid.wasDblClicked(e||window.event)){return false};if(this.grid._dclE)this.grid.editCell(e||window.event);(e||event).cancelBubble=true;");
	...
	// dhtmlxEvent(document.body,"click",function(){if(self.editStop)self.editStop();return
	// true;});

	this.entBox.onbeforeactivate = new Function("",
			"this.grid.setActive();event.cancelBubble=true;");
	this.entBox.onbeforedeactivate = new Function("",
			"this.grid.isActive=-1;event.cancelBubble=true;");
...
dhtmlXGridObject.prototype._addRow = function(new_id, text, ind) {
	...
	if (ind > this.rowsCol.length
			&& ind < (this.rowsCol.length + this.rowsBuffer[0].length)) {
...
dhtmlXGridObject.prototype.setOnLeftClick = function(func) {
	this.dhx_attachEvent("onLCL", func);
};
...
dhtmlXGridObject.prototype._aplNF = function(data, ind) {
	...
	data = Math.abs(
			Math.round(parseFloat(data) * Math.pow(10, a[0] > 0 ? a[0] : 0)))
			.toString();
...
dhtmlXGridObject.prototype._launchCommands = function(arr) {
	for (var i = 0; i < arr.length; i++) {
		...
		for (var j = 0; j < arr[i].childNodes.length; j++)
			...
			if (arr[i].childNodes[i].nodeType == 3) {
				if (arr[i].childNodes[j].firstChild) {
					if (arr[i].childNodes[j].firstChild.data) {
						args[args.length] = arr[i].childNodes[j].firstChild.data;
						// console.log(arr[i].childNodes[j].firstChild);
					}
				}
...
dhtmlXGridObject.prototype._parseHead = function(hheadCol) {
	...
	this._launchCommands(this.xmlLoader.doXPath("//rows/head/beforeInit/call",
			hheadCol[0]));
	...
	if (headCol.length > 0) {
		...
		for (var i = 0; i < headCol.length; i++) {
			...
			e_list += (headCol[i].getAttribute("color") != null ? headCol[i]
					.getAttribute("color") : "")
					+ ",";
	...
	this._launchCommands(this.xmlLoader.doXPath("//rows/head/afterInit/call",
			hheadCol[0]));
}
...
dhtmlXGridObject.prototype.parseXML = function(xml, startIndex) {
	...
	if (rowsCol.length == 0) {
		...
		if ((tree != -1) && (this.rowsAr[pid])) {
			...
			tree_r.innerHTML = tree_r.innerHTML.replace(/\/(plus)\.gif/,
					"/blank.gif");
		}
	} else {
...
dhtmlXGridObject.prototype._postRowProcessing = function(r, xml) {
	...
	if (udCol.length > 0) {
		...
		for (var j = 0; j < udCol.length; j++) {
			this.UserData[rId].put(udCol[j].getAttribute("name"),
					udCol[j].firstChild ? udCol[j].firstChild.data : "");
		}
	}
	...
	if (xml.getAttribute("selected") == true) {
		this.setSelectedRow(rId, this.selMultiRows, false, xml
...
dhtmlXGridObject.prototype._fillRowFromXML = function(r, xml, tree, pId) {
	...
	for (var j = 0; j < cellsCol.length; j++) {
		...
		if (j != tree)
			...
			strAr[strAr.length] = pId + "^" + cellVal + "^"
					+ ((xml.getAttribute("xmlkids") || r._xml) ? "1" : "0")
...
dhtmlXGridObject.prototype.getCheckedRows = function(col_ind) {
	...
	for (var i = 0; i < this.rowsCol.length; i++) {
		if (this.rowsCol[i]) {
			var cell = this.cells3(this.rowsCol[i], col_ind);
			if (cell.isCheckbox()) {
				var v = cell.getValue();
				if (v != "0" && v != "")
...
dhtmlXGridObject.prototype._drawTooltip = function(e) {
	...
	if (ced)
		(e ? e.target : event.srcElement).title = ced.getTitle ? ced.getTitle()
...
dhtmlXGridObject.prototype.adjustColumnSize = function(cInd) {
	...
	for (var i = 0; i < l; i++) {
		if (_isFF || _isOpera)
			var z = this.obj._rows(i).childNodes[cInd].innerHTML.replace(
					/<[^>]*>/g, "").length * 7;
		else
...
dhtmlXGridObject.prototype.attachHeader = function(values, style, _type) {
	...
	// alert(values +'_'+style +'_'+_type); alert(style); alert(style);

	if (typeof (values) == "string")
		values = values.split(this.delim);
...
dhtmlXGridObject.prototype.attachFooter = function(values, style) {
	// alert('attachFooter values>>>'+values);
	// alert('attachFooter style>>>'+style);
	...
	if (!skip) {
		...
		dhtmlXGridObject.prototype.render_dataset = function(min, max) {
			if (this._srnd) {
				...
				max = Math.min((this._get_view_size() + (this._srnd_pr || 0)),
						this.rowsBuffer.length)
			}
			;
			if (this.pagingOn) {
				min = Math.max((min || 0), (this.currentPage - 1)
						* this.rowsBufferOutSize);
				max = Math.min(this.currentPage * this.rowsBufferOutSize,
						this.rowsBuffer.length)
			} else {
				min = min || 0;
			...
			for (var i = min; i < max; i++) {
				...
				if (!r.parentNode || !r.parentNode.tagName) {
					...
					if (r._attrs["selected"] || r._attrs["select"]) {
						this
								.selectRow(r, r._attrs["call"] ? true : false,
										true);
				...
				if (this._ads_count && i - min == this._ads_count) {
					var that = this;
					this._context_parsing = this._context_parsing
							|| this._parsing;
					return this._contextCallTimer = window.setTimeout(
							function() {
...
dhtmlXGridObject.prototype._in_header_master_checkbox = function(tag, index,
		data) { // the name contains "_in_header_"+shortcut_name
	tag.innerHTML = data[0] + "<input type='checkbox' />"; // HTML view
	var grid = this; // store reference for further usage
	tag.firstChild.onclick = function() {

		var val = this.checked ? 1 : 0; // on button click
		if (val == 0) {
			grid.clearSelection();
			grid.uncheckAll();
			grid.forEachRow(function(id) {
				grid.getRowById(id).className = grid.getRowById(id).className
						.replace(/rowselected2/g, "");
			});
		} else {
			grid.checkAll();
			grid.forEachRow(function(id) {
				grid.getRowById(id).className += " rowselected2";
				//					 	  mygrid1.setSelectedRow(id,mygrid1.selectedRows);
			});
		}

		//              grid.forEachRow(function(id){               	// for each row in the grid
		//                  grid.cells(id,index).setValue(val);       // clear cell value for the related column
		//                  if(val == 0){
		//                  	  grid.getRowById(id).className = grid.getRowById(id).className.replace(/rowselected2/g,"");
		//                  }else{
		//                  	  mygrid1.setSelectedRow(id,mygrid1.selectedRows);
		//                  	  grid.getRowById(id).className += " rowselected2";
		//                  }
...
dhtmlXGridObject.prototype._xml_attrs = function(node) {
	var data = {};
...
//	this.forEachRowA(function(id){if(this.cells(id,cInd).isCheckbox())this.cells(id,cInd).setValue(v)});
//};
