 /* 扩展的功能点有：   
 * �?、支持只对树的叶子进行�?�择   
 *    只有当返回的树结点属性leaf = true 时，结点才有checkbox可�??   
 *    使用时，只需在声明树时，加上属�?? onlyLeafCheckable: true 既可，默认是false   
 *    
 * 二�?�支持对树的单�??   
 *    只允许�?�择�?个结�?   
 *    使用时，只需在声明树时，加上属�?? checkModel: "single" 既可   
 *    
 * 二�?�支持对树的级联多�??    
 *    当�?�择结点时，自动选择该结点下的所有子结点，以及该结点的所有父结点（根结点除外），特别是支持异步，当子结点还没显示时，会从后台取得子结点，然后将其选中/取消选中   
 *    使用时，只需在声明树时，加上属�?? checkModel: "cascade" 既可   
 *    
 * 三�?�添�?"check"事件   
 *    该事件会在树结点的checkbox发生改变时触�?   
 *    使用时，只需给树注册事件,如：   
 *    tree.on("check",function(node,checked){...});   
 *   
 * 默认情况下，checkModel�?'multiple'，也就是多�?�，onlyLeafCheckable为false，所有结点都可�??   
 *    
 * 使用方法：在loader里加�? baseAttrs:{uiProvider:Ext.tree.TreeCheckNodeUI} 既可.   
 * 例如�?   
 *   var tree = new Ext.tree.TreePanel({   
 *      el:'tree-ct',   
 *      width:568,   
 *      height:300,   
 *      checkModel: 'cascade',   //对树的级联多�?   
 *      onlyLeafCheckable: false,//对树�?有结点都可�??   
 *      animate: false,   
 *      rootVisible: false,   
 *      autoScroll:true,   
 *      loader: new Ext.tree.DWRTreeLoader({   
 *          dwrCall:Tmplt.getTmpltTree,   
 *          baseAttrs: { uiProvider: Ext.tree.TreeCheckNodeUI } //添加 uiProvider 属�??   
 *      }),   
