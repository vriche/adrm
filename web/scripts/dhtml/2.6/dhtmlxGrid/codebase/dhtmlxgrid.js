	...
	this.setSkin = function(name) {
		...
		case "clear":
			...
			this._topMb.innerHTML = "<img style='left:0px' src='" + this.imgURL
					+ "skinC_top_left.gif'><img style='right:20px' src='"
			...
			this._botMb.innerHTML = "<img style='left:0px' src='" + this.imgURL
					+ "skinD_bottom_left.gif'><img style='right:20px' src='"
	...
	this.setColumnSizes = function(gridWidth) {
		...
		for (var i = 0; i < this._cCount; i++) {
			...
			if (this.cellWidthType == '%') {
				...
				this.cellWidthPX[i] = Math.floor(gridWidth
						* this.cellWidthPC[i] / 100) || 0;
			} else {
		...
		if (fcols.length) {
			...
			for (var i = 0; i < fcols.length; i++) {
				var next = Math.max((this._drsclmW ? this._drsclmW[fcols[i]]
						: 0), ms)
	...
	this.setSizes = function() {
		...
		if (this.globalBox) {
			...
			if (this._delta_x && !this._realfake) {
				...
				this.entBox.style.width = Math
						.max(
								0,
								(this.globalBox.clientWidth + (quirks ? splitOuterBorder * 2
										: 0))
		...
		;
		var border_x = (((this.entBox.cmp || this._delta_x)
				&& (this.skin_name || "").indexOf("dhx") == 0 && !quirks) ? 2
				: 0);
		var border_y = (((this.entBox.cmp || this._delta_y)
				&& (this.skin_name || "").indexOf("dhx") == 0 && !quirks) ? 2
				: 0);
		...
		this.entBox.style.width = newWidth + (quirks ? 2 : 0) * outerBorder
				+ this._ff_size_delta + "px";
		this.entBox.style.height = newHeight + (quirks ? 2 : 0) * outerBorder
	...
	this.setColTypes = function(typeStr) {
		...
		for (var i = 0; i < this.cellType.length; i++) {
			...
			if (!window["eXcell_" + this.cellType[i]])
				dhtmlxError.throwError("Configuration", "Incorrect cell type: "
						+ this.cellType[i], [ this, this.cellType[i] ]);
		}
	};
...
dhtmlXGridObject.prototype = {
	...
	_parseHead : function(xmlDoc) {
		...
		if (hheadCol.length) {
			...
			;
			this._launchCommands(this.xmlLoader.doXPath("./beforeInit/call",
					hheadCol[0]));
			if (headCol.length > 0) {
				if (this.hdr.rows.length > 0)
					this.clearAll(true);
				...
				var attrs = [ "", "width", "type", "align", "sort", "color",
						"format", "hidden", "id" ];
				var calls = [ "", awidthmet, "setColTypes", "setColAlign",
						"setColSorting", "setColumnColor", "", "",
						"setColumnIds" ];
			...
			;
			this._launchCommands(this.xmlLoader.doXPath("./afterInit/call",
					hheadCol[0]));
		}
		;
			...
			if (typeof data[0][i] == "object")
				...
				if ((data[0][i] || "").indexOf("#") != -1) {
					...
					if (t) {
						...
						if (this[tn])
							this[tn](
									(this.forceDivInHeader ? w.firstChild : w),
									i, data[0][i].split(t[0]));
					}
				}
			}
			;
	...
	enableAutoWidth : function(mode, max_limit, min_limit) {
		this._awdth = [ convertStringToBoolean(mode),
				parseInt(max_limit || 99999), parseInt(min_limit || 0) ];
		if (arguments.length == 1)
			...
			if (this.rowsAr[id] && this.rowsAr[id].tagName != "TR") {
				...
				if (this._h2) {
					...
					(this._h2
							.add(
									id,
									(row.parentNode.getAttribute("id") || row.parentNode
				...
				if (this.xmlFileUrl) {
					if (this.callEvent("onDynXLS", [ i,
						...
						this.load(this.xmlFileUrl
								+ getUrlSymbol(this.xmlFileUrl) + "posStart="
								+ i + "&count="
								+ (this._dpref ? this._dpref : (max - i)),
								this._data_type);
				}
		...
		if (this._fillers) {
			...
			for (var i = 0; i < this._fillers.length; i++) {
				...
				if (f && f[0] <= ind && (f[0] + f[1]) >= ind) {
					f[1] = f[1] + 1;
					f[2].firstChild.style.height = parseInt(f[2].firstChild.style.height)
							+ this._srdh + "px";
					found = true;
					if (this._fake)
	...
	setExternalTabOrder : function(start, end) {
		...
		this.tabEnd.onkeydown = this.tabEnd.onkeypress = function(e) {
			...
			if ((ev.keyCode == 9) && ev.shiftKey) {
				...
				if (grid.smartTabOrder
						&& grid.cells2((grid.getRowsNum() - 1),
	...
	getStateOfView : function() {
		if (this.pagingOn) {
			...
			return [
					this.currentPage,
					start,
					Math.min(start + this.rowsBufferOutSize,
							this.rowsBuffer.length), this.rowsBuffer.length ];
		}
		;
		...
		columns : function(obj) {
			for (var j = 0; j < columns_map.length; j++) {
				...
				for (var i = 0; i < obj.length; i++)
					settings[i] = obj[i][columns_map[j].name]
							|| columns_map[j].def;
...
dhtmlXGridObject.prototype._dp_init = function(dp) {
	...
	dp._methods = [ "setRowTextStyle", "setCellTextStyle", "changeRowId",
			"deleteRow" ];
	...
	dp.checkBeforeUpdate = function(rowId) {
		...
		for (var i = 0; i < this.obj._cCount; i++)
			if (this.mandatoryFields[i]) {
				var res = this.mandatoryFields[i].call(this.obj, this.obj
						.cells(rowId, i).getValue(), rowId, i);
