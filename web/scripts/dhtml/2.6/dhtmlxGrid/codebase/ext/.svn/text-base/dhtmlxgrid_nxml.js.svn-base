...
/* 그리드의 내용을 엑셀 화일로 저장하는 함수 입니다. */
var bAddedForm = false;
var excelDivId;
dhtmlXGridObject.prototype.enableExportToExcel = function(DivId)
{
    excelDivId = DivId;
}
function saveToExcel(excelGrid){
    
    
    excelGrid.enableCSVHeader(true);
    excelGrid.setCSVDelimiter(",");//chnage csv delimiter
    excelGrid.setSerializableColumns();
    cvsString = excelGrid.serializeToCSV();//    
    if(bAddedForm == false)
    {
    document.getElementById(excelDivId).innerHTML = "<form name='excelForm' method='post' action='../convert/Convert_CSVFile.php'><input type='hidden' name='CSVFile'></form>";
    bAddedForm = true;
    }                
    document.getElementById('excelForm').CSVFile.value = cvsString;
    document.excelForm.submit();     
    
}

/* edncl 과 ednro를 사용할수있게 하는 평션입니다. */
//mygrid.setNumberFormat("$0,000.00",10);
/* 사용하시려면 코드에 위 명령어를 추가하고 타입을 edncl or ednro를 사용하시면됩니다.  */  
