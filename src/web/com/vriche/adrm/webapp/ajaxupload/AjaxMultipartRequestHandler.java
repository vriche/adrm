...
/**
 * This class implements the <code>MultipartRequestHandler</code> interface by providing a wrapper around the Jakarta
 * Commons FileUpload library.
 *
 * @since Struts 1.1
 */

public class AjaxMultipartRequestHandler implements MultipartRequestHandler {

	// ----------------------------------------------------- Manifest Constants

	/**
	 * The default value for the maximum allowable size, in bytes, of an uploaded file. The value is equivalent to
	 * 250MB.
	 */
	 ...
	 */
	protected static final Log log = LogFactory.getLog(CommonsMultipartRequestHandler.class);
	...
	public void handleRequest(HttpServletRequest request) throws ServletException {
		...
		factory.setRepository(new File(getRepositoryPath(ac)));
...
//		for(Iterator item1 = items.iterator();item1.hasNext();){
//			FileItem item = (FileItem) item1;
//			
//						if (item.isFormField()) {
//							addTextParameter(request, item);
...
//	}
	...
	public void rollback() {
		...
		for(Iterator it = elementsFile.values().iterator();it.hasNext();){
			FormFile formFile = (FormFile) it;
			formFile.destroy();			
		}	
		
	
//        for(int i=0;i<elementsFile.size();i++){
//            FileItem item = (FileItem)elementsFile.get(i);
//            formFile.destroy();		
//        }			
//		
	...
	/**
	 * Returns the maximum allowable size, in bytes, of an uploaded file. The value is obtained from the current
	 * module's controller configuration.
	 *
	 ...
	 * <p/>
	 * If the size value cannot be converted, for
	 * example due to invalid syntax, the supplied default is returned instead.
	 *
	 ...
	 * <ol>
	 * <li>A temp dir explicitly defined either using the <code>tempDir</code> servlet init param, or the
	 * <code>tempDir</code> attribute of the &lt;controller&gt; element in the Struts config file.</li>
	 * <li>The container-specified temp dir, obtained from the <code>javax.servlet.context.tempdir</code> servlet
	 * context attribute.</li>
	 * <li>The temp dir specified by the <code>java.io.tmpdir</code> system property.</li>
	 * (/ol>
	 *
	 * @param mc The module config instance for which the path should be determined.
	...
	static class CommonsFormFile implements FormFile, Serializable {
		...
		/**
		 * Sets the content type for this file. <p/> NOTE: This method is not supported in this implementation.
		 *
		...
		/**
		 * Sets the size, in bytes, for this file. <p/> NOTE: This method is not supported in this implementation.
		 *
		...
		/**
		 * Sets the (client-side) file name for this file. <p/> NOTE: This method is not supported in this
		 * implementation.
		 *
		 ...
		 * @throws FileNotFoundException If some sort of file representation cannot be found for the FormFile
		 ...
		 * task. Apparently, however, some Linux JDKs do not implement <code>File.getName()</code> correctly for
		 * Windows paths, so we attempt to take care of that here.
		 *
