 ...
 * 
 */
public class OrderDetailBak extends BaseObject { 

	/**
	 * 
	 */
	private static final long serialVersionUID = -141647042821891042L;
	protected Long id;
...
//    protected CustomerProduct customerProduct = new CustomerProduct();
	...
	*/    
	...
	public String toString() {
		return new ToStringBuilder(this).append("isSaveOrderDayInfo",
				this.isSaveOrderDayInfo).append("moneyBase", this.moneyBase)
				.append("carrierId", this.carrierId).append("orderPublic",
						this.orderPublic).append("modifyDate", this.modifyDate)
				.append("matterLength", this.matterLength).append(
						"resourceSpecificId", this.resourceSpecificId).append("createBy",this.createBy)
				.append("execPrice", this.execPrice).append("contractId",
						this.contractId)
				.append("sumTimes", this.sumTimes).append("orderDayInfosBak",
						this.orderDayInfosBak).append("appRate", this.appRate)
				.append("isSpaceAdver", this.isSpaceAdver).append("isCkecked", this.isCkecked).append("moneyBalance",
						this.moneyBalance)
				.append("ageRate", this.ageRate).append("moneyRealpay",
						this.moneyRealpay).append("modifyBy", this.modifyBy)
				.append("id", this.id).append("orderCategoryMain",
						this.orderCategoryMain).append("isNotInSeries",
						this.isNotInSeries).append("parentId", this.parentId)
				.append("publishMemo", this.publishMemo).append("resourceType",this.resourceType).append("matterId", this.matterId)
				.append("memo", this.memo).append("resourceSortId",
						this.resourceSortId).append("orderCategoryId",
						this.orderCategoryId).append("industryTypeId",
						this.industryTypeId).append("publishEndDate",
						this.publishEndDate)
				.append("paymentId", this.paymentId).append("resourceInfoId",
						this.resourceInfoId).append("moneyIn", this.moneyIn)
				.append("orderId", this.orderId).append("favourRate",
						this.favourRate).append("compagesId", this.compagesId)
				.append("version", this.version).append("needPublish",
						this.needPublish)
				.append("sysPrice", this.sysPrice).append("publishStartDate",
