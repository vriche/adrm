...
//    }
    ...
    public String saveObjectRt(final Object object) {
        String className = ClassUtils.getShortName(object.getClass());
        Object primaryKey = getPrimaryKeyValue(object);
        String keyId = null;
        
        System.out.println("saveOrderClone    className>>>>>>>"+ className);
        System.out.println("saveOrderClone    primaryKey>>>>>>>"+ primaryKey);
        // check for null id
        if (primaryKey != null) {
            keyId = primaryKey.toString();
        }
        
        System.out.println("saveOrderClone    keyId>>>>>>>"+ keyId);
        
        

        // check for new record
        if (StringUtils.isBlank(keyId)||"0".equals(keyId)) {  
            prepareObjectForSaveOrUpdate(object);
            primaryKey = getSqlMapClientTemplate().insert(getInsertQuery(className), object);
	...
	public DataSource getDefaultDataSource() {
		...
		return getSqlMapClientTemplate().getSqlMapClient().getDataSource();
